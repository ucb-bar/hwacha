...
//-------------------------------------------------------------------------\\
  ...
  val forward = (state === VXU_FORWARD) && (!decode_fence_cv && !decode_fence_v || !io.hazard_to_issue.pending_memop)
  ...
  io.vmu_vcmdq.valid := 
    tvec_active && 
    (state === VXU_FORWARD) && (decode_fence_cv || decode_fence_v) && !io.hazard_to_issue.pending_memop &&
