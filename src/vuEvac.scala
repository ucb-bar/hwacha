...
class vuEvac extends Module
{
  ...
  val cmd_sel = Reg(update = cmd_sel_next, resetVal = SEL_CMDB)
  ...
  val state = Reg(update = state_next, resetVal = STATE_IDLE)
  val addr_reg = Reg(update = addr_next, resetVal = UInt(0, SZ_ADDR))
