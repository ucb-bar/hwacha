...
class updateReq(DATA_SIZE: Int, ADDR_SIZE: Int) extends Bundle 
{
...
class io_buffer(DATA_SIZE: Int, ADDR_SIZE: Int) extends Bundle 
{
  ...
  val update = new io_valid()( new updateReq(DATA_SIZE, ADDR_SIZE) ).flip()
...
class Buffer(DATA_SIZE: Int, DEPTH: Int, usePrevPtr: Boolean = false) extends Component 
{
  ...
  when (do_enq && !do_deq && (write_ptr_next === deq_ptr))
  {
    full_next := Bool(true)
  }
  ...
  val empty = !full && (deq_ptr === enq_prt)

  val data_next = GenArray(DEPTH){ Wire(){ width=DATA_SIZE } }
  val data_array = GenAarray(DEPTH){ Reg(resetVal = Bits(0, DATA_SIZE) } }
