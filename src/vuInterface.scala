...
class io_vcmdq extends FIFOIO()( { Bits(width = SZ_VCMD) } )
class io_vximm1q extends FIFOIO()( { Bits(width = SZ_VIMM) } )
class io_vximm2q extends FIFOIO()( { Bits(width = SZ_VSTRIDE) } )
class io_vcntq extends FIFOIO()( Bits(width=SZ_VLEN+1) )
