...
{
  val vaq_deq = new io_ready_valid()({ new io_vaq_bundle() })
  val vaq_ack = Bool(OUTPUT)
  val vaq_nack = Bool(OUTPUT)

  val vsdq_deq = new io_ready_valid()({ Bits(width = 65) })
  val vsdq_ack = Bool(OUTPUT)
  val vsdq_nack = Bool(OUTPUT)

  val vldq_deq_rtag = new io_ready_valid()({ Bits(width = 8) })
