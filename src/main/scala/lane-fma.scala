...
class LaneFMA(implicit conf: HwachaConfiguration) extends Module
{
  ...
  val one_dp = repack_d(Bits("h8000000000000000", SZ_FPD)) // recoded, swizzled
  val one_sp = repack_w(Bits("h080000000", SZ_FPS), Bits("h080000000", SZ_FPS)) // recoded, confprec'd, swizzled
  val one_hp = repack_h(Bits("h3c00", SZ_FPH), Bits("h3c00", SZ_FPH), Bits("h3c00", SZ_FPH), Bits("h3c00", SZ_FPH)) // not recoded, confprec'd, swizzled
  ...
  for (i <- 0 until N_FPD) {
    if (conf.mixedprec || i == 0) {
      ...
      dfma.io.a := Fill(65,val_fma_dp) & unpack_d(fma_multiplicand, i)
      dfma.io.b := Fill(65,val_fma_dp) & unpack_d(fma_multiplier, i)
      dfma.io.c := Fill(65,val_fma_dp) & unpack_d(fma_addend, i)
  ...
  for (i <- 0 until N_FPS) {
    if (conf.mixedprec || i == 0) {
      ...
      sfma.io.a := Fill(33,val_fma_sp) & unpack_w(fma_multiplicand, i)
      sfma.io.b := Fill(33,val_fma_sp) & unpack_w(fma_multiplier, i)
      sfma.io.c := Fill(33,val_fma_sp) & unpack_w(fma_addend, i)
  ...
  for (i <- 0 until N_FPH) {
    if (conf.mixedprec || i == 0) {
      val recoded_hp_a = hardfloat.floatNToRecodedFloatN(unpack_h(fma_multiplicand, i).toUInt, 10, 6)
      val recoded_hp_b = hardfloat.floatNToRecodedFloatN(unpack_h(fma_multiplier, i).toUInt, 10, 6)
      val recoded_hp_c = hardfloat.floatNToRecodedFloatN(unpack_h(fma_addend, i).toUInt, 10, 6)
  ...
  if (conf.mixedprec) {
    result := MuxCase(
      ...
      (val_fma_dp) -> Cat(or_exc_dp, repack_d(dfmas:_*)),
      (val_fma_sp) -> Cat(or_exc_sp, repack_w(sfmas:_*)),
      (val_fma_hp) -> Cat(or_exc_hp, repack_h(hfmas:_*))
      ))
  } else {
    result := MuxCase(
      Bits("h3FFFFFFFFFFFFFFFFF",71), Array(
      (val_fma_dp) -> Cat(or_exc_dp, repack_d(dfmas(0))),
      (val_fma_sp) -> Cat(or_exc_sp, repack_w(sfmas(0))),
      (val_fma_hp) -> Cat(or_exc_hp, repack_h(hfmas(0)))
      ))
  }
