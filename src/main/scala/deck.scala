...
abstract trait HwachaDeckParameters extends UsesParameters
{
  val nb = params(HwachaNBanks)
}

class Deck(resetSignal: Bool = null) extends HwachaModule(_reset = resetSignal) with HwachaDeckParameters {
  val io = new Bundle {
    ...
    val brqs = Vec.fill(nb){new BRQIO().flip}
    val bwqs = Vec.fill(nb){new BWQIO}
...
class VLU extends VMUModule with HwachaDeckParameters {
  val io = new Bundle {
    val cfg = new HwachaConfigIO().flip
    val op = new DeckOpIO().flip
  ...
  val src_data_d = Vec.tabulate(tlDataDoubles){ i => {
    val elt = unpack(SZ_D, i)
    val elt_rf = hardfloat.floatNToRecodedFloatN(elt, 52, 12)
    Mux(op.float, elt_rf, prefix(elt, SZ_D))
  }}

  val src_data_w = Vec.tabulate(tlDataWords){ i => {
    val elt = unpack(SZ_W, i)
    val elt_rf = hardfloat.floatNToRecodedFloatN(elt, 23, 9)
    Mux(op.float, elt_rf, prefix(elt, SZ_W))
  }}

  val src_data_h = Vec.tabulate(tlDataHalves){ i => unpack(SZ_H, i) }
  val src_data_b = Vec.tabulate(tlDataBytes/2){ i => unpack(SZ_B, i) }

  //--------------------------------------------------------------------\\
  // permutation network
  //--------------------------------------------------------------------\\

  private val lgbank = log2Up(nb)
  ...
  private def permute[T <: Data](src: Vec[T], sel: UInt, rev: Boolean = false): Vec[T] = {
    require(src.size > 0)
    val rot = Module(new Rotator(src(0).clone, src.size, math.max(src.size, nb), rev))
    val dst = Vec.fill(nb){ src(0).clone }
    rot.io.in := src
    rot.io.sel := sel
    for (i <- 0 until nb) {
  ...
  val src_en_raw = EnableDecoder(meta.ecnt, nb)
  val src_en_bits = Cat(src_en_raw.reverse) << meta.eskip
  val src_en = Vec.tabulate(nb)(i => src_en_bits(i))
  ...
  val bwqs = Vec.tabulate(nb){ i => {
    ...
    bwq.io.enq.bits.data := Mux1H(mt_seq, Seq(
      extend(dst_data_b(i), SZ_B, recoded=false),
      extend(dst_data_h(i), SZ_H, recoded=false),
      extend(dst_data_w(i), SZ_W, recoded=true),
      extend(dst_data_d(i), SZ_D, recoded=true)))
...
class LoadRepacker extends VMUModule with HwachaDeckParameters {
  val io = new Bundle {
  ...
  val ecnt_head = SInt(nb) - meta.eskip.zext
  ...
  io.deq.bits.data := Mux(shift,
    io.enq.bits.data(tlDataBits-1, nb*8),
    io.enq.bits.data)
      ...
      is (s1) {
        when (has_tail) {
          when (has_head) { /* two beats */
            ...
            io.deq.bits.meta.eskip := meta.eskip - UInt(nb)
            shift := Bool(true)
          }
        }
      }
...
class VSU extends VMUModule with HwachaDeckParameters {
  val io = new Bundle {
    val cfg = new HwachaConfigIO().flip
    val op = new DeckOpIO().flip
  ...
  val sla_en = EnableDecoder(io.la.cnt(lgbank,0), nb)
  val sla_avail = Vec.fill(nb)(Bool())
  io.la.available := sla_avail.reduce(_&&_)

  val cnt_sz = log2Down(nb*nbrq)+1
  ...
  val brqs_en_raw = EnableDecoder(ecnt, nb)
  ...
  val brqs_en = Vec.tabulate(nb)(i => brqs_en_bits(i))
