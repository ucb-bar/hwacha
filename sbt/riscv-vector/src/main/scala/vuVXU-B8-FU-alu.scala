...
class vuVXU_Banked8_FU_alu extends Component {
  ...
  val adder_out = (Cat(reg_in0(63, 0), sub).toUFix + Cat(reg_in1(63, 0) ^ Fill(64, sub), sub).toUFix)(64, 1);

  val shamt = MuxCase(UFix(0, 6), Array(
  ...
  val fminmax = 
    Mux(is_in1_nan || ~is_in0_nan && (want_min === less_fp), reg_in0,
				     reg_in1)
