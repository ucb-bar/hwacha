...
class recodedFloat64ToAny(SIG_WIDTH: Int = 52, EXP_WIDTH: Int = 12, INT_WIDTH: Int = 64) extends Component {
  override val io = new recodedFloat64ToAny_io(SIG_WIDTH, EXP_WIDTH, INT_WIDTH);
	...
	val EXP_OFFSET = Bits("h800", 12).toUFix; //12'h800; // Recoded offset=2048 (IEEE offset=1023)
...
val isValidShift = (exponent(11,6) === Bits("b100000",6) || isZeroOrOne);
val shift_count =
	Mux(exponent(11,6) === Bits("b100000",6),  Cat(Bits(0,1),~exponent(5,0)),
...
val isValidSigned =
	Mux(isValidShift && !sign, (absolute_round(INT_WIDTH-1,0) <= maxInteger), // BCR: LE Op OK?
	Mux(isValidShift && sign,  (absolute_round(INT_WIDTH-1,0) <= maxInteger+UFix(1)), // BCR: LE Op OK?
	isTiny));
