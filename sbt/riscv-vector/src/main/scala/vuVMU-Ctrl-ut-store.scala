package riscvVector {
  ...
  class vuVMU_Ctrl_ut_storeIO extends Bundle {
    val stcmdq_deq_bits		= UFix(interface.UT_STCMD_SZ, 'input);
  ...
  class vuVMU_Ctrl_ut_store extends Component {
    ...
    val vlen          = io.stcmdq_deq_bits(interface.UTMCMD_VLEN_SZ-1,0);
    val addr          = io.stcmdq_deq_bits(interface.UTMIMM_SZ+interface.UTMCMD_VLEN_SZ-1, interface.UTMCMD_VLEN_SZ);
    val cmd_type      = io.stcmdq_deq_bits(interface.UT_STCMD_SZ-2, interface.UT_STCMD_SZ-6);
    val cmd_type_amo  = io.stcmdq_deq_bits(interface.UT_STCMD_SZ);
